...
namespace InfrastructureTests
{
    public class VehicleRepositoryTests : BaseRepositoryTests
    {
        ...
        public async Task MustUpdateVehicle()
        {
            ...
            var updatedVehicle = await VehicleRepository.GetVehicle(id: ExistingVehicleId);

            Assert.That(registry, Is.Not.EqualTo(0));
            Assert.That(updatedVehicle, Is.Not.Null);
            Assert.Multiple(() =>
            {
                Assert.That(updatedVehicle.Model, Is.EqualTo(VehicleToUpdate.Model));
                Assert.That(updatedVehicle.Year, Is.EqualTo(VehicleToUpdate.Year));
            });
        }
