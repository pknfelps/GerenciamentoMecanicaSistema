...
- Em 2026-09-18, configuração Compose revalidada após remoção do SQL automático. Tentativa de build da imagem bloqueada antes da compilação: Docker Desktop falhou ao iniciar seu Inference manager (`dockerInference` inacessível). Validação do novo job de imagem permanece pendente; nenhuma alteração no código .NET, sem repetição da suíte de 468 testes nesta revisão.
