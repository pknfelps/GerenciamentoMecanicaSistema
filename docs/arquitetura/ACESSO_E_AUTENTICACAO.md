...
| Cliente | Validar o CPF pela função serverless; consultar o status e aprovar/recusar o orçamento da própria OS |
...
Rotas de emissão de token não exigem token prévio: `/authentication` valida credenciais internas; `/customers/validate` valida CPF/cadastro/status. Enviar um token a elas não dispensa essas verificações. A API mantém as validações de domínio e de estado da OS após a autorização.
...
## 5. POST /customers/validate — validação serverless do CPF
...
CPF ilustrativo para contrato/teste, sem associação a pessoa real.
...
| 401 | CPF válido sem cliente elegível: cadastro inexistente ou Inativo |
...
- Gerar claims a partir do cadastro consultado. Payload de validação de CPF não escolhe ID, role, issuer, audience ou expiração.
...
- Validar CPF com/sem máscara, formato/dígitos inválidos, CNPJ, cadastro ausente, Ativo e fixture Inativo.
