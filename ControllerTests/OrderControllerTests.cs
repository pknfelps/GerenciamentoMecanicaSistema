...
namespace ControllerTests
{
    public class OrderControllerTests : BaseControllerTests
    {
        ...
        protected override void MockService()
        {
            ...
            {
                var command = callInfo.ArgAt<CreateOrderCommand>(0);

                if (AreEquivalent(command, OrderToCreate))
        ...
        public async Task MustCreateOrder()
        {
            ...
            await OrderService.Received(1).CreateServiceOrder(
                Arg.Is<CreateOrderCommand>(command => AreEquivalent(command, OrderToCreate)));
        ...
        public async Task MustCreateOrderWithoutServicesAndMaterials()
        {
            OrderService.CreateServiceOrder(
                Arg.Is<CreateOrderCommand>(command =>
                    command.CustomerId == ExistingCustomerId
                    && command.VehicleId == ExistingVehicleId
                    && command.Services.Count == 0
                    && command.Materials.Count == 0))
        ...
        public async Task MustReturnBadRequestIfTryCreateOrderWithInvalidModel()
        {
            ...
            await OrderService.ReceivedWithAnyArgs(0).CreateServiceOrder(default!);
        ...
        public async Task MustReturnNotFoundIfTryCreateOrderWithInvalidOrder()
        {
            ...
            await OrderService.ReceivedWithAnyArgs(1).CreateServiceOrder(default!);
        ...
        private static bool AreEquivalent(CreateOrderCommand command, CreateOrderRequest request) =>
            command.CustomerId == request.CustomerId
            && command.VehicleId == request.VehicleId
            && command.Services.SequenceEqual(request.Services?.Select(service => service.ToCommand()) ?? [])
            && command.Materials.SequenceEqual(request.Materials?.Select(material => material.ToCommand()) ?? []);
    }
}
